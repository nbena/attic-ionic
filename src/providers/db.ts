...
private static getLogsForNotesFromDbResult(res:any):LogObjSmart[]{
...
private getLogsForNotes(title:string, userid:string, tx?:any):Promise<LogObjSmart[]>{
  return new Promise<LogObjSmart[]>((resolve, reject)=>{
    let logs:LogObjSmart[]=[];
    if(tx!=null){
      tx.executeSql(Query.GET_LOGS_BY_NOTE, [userid, title],
        (tx:any, res:any)=>{
          logs = Db.getLogsForNotesFromDbResult(res);
          resolve(logs);
...
*/
private removeTagsFromNoteCore(tx:any, extraMin:NoteExtraMin, userid:string,tags:TagExtraMin[], usedTag?:TagFull[]){
  let tmpTag:TagExtraMin[]=Utils.binaryArrayDiff(tags, ((usedTag)==null) ? [] : usedTag, TagExtraMin.ascendingCompare);
...
private addTagsToNoteUpdateOnlyTags(tx:any, extraMin:NoteExtraMin, userid:string,tags:TagExtraMin[], usedTag?:TagFull[]){
  ...
  //updateTag contains the list of the tag that will effectively be updated.
  //which is, beasically, just a matter of adding the note to the notes array and increment
  //the count.
  //
  // console.log('tmp tag');console.log(JSON.stringify(tmpTag));
  ...
  console.log('finally, the tag to update:');
  console.log(JSON.stringify(updatedTag));
  if(updatedTag.length>0){
    let query:string = this.prepareQueryInsertMultiTags(updatedTag.length);
...
public createNewNote2(note:NoteFull, userid:string, usedTag?:TagFull[]):Promise<void>{
  ...
  return new Promise<void>((resolve, reject)=>{
    this.db.transaction(tx=>{
      ...
      // );
      // }
      this.addTagsToNoteUpdateOnlyTags(tx, extraMin, userid, tags, usedTag); //test-this.
...
public insertTag(tag:TagFull, userid:string):Promise<void>{
  return new Promise<void>((resolve, reject)=>{
    ...
    //   reject(error);
    // })
    return this.updateJsonObjTag(tag, userid);
  })
}
...
private updateJsonObjNote(fullNote:NoteFull, userid:string, tx?:any):void|Promise<void>{
  ...
  if(tx!=null){
    ...
    return this.db.executeSql(Query.UPDATE_JSON_OBJ_NOTE, [json, fullNote.title, userid]);
  }
}
...
private updateJsonObjTag(fullTag:TagFull, userid:string, tx?:any):void|Promise<void>{
  ...
  if(tx!=null){
    ...
    return this.db.executeSql(Query.UPDATE_JSON_OBJ_TAG, [json, fullTag.title, userid]);
  }
}
